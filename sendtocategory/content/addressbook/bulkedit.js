...
jbCatMan.checkInput = function () {
  ...
  for (let j=0; j<jbCatMan.data.validatorFields.length; j++) {
    checkField = document.getElementById(jbCatMan.data.validatorFields[j]);
  ...
  if (i < jbCatMan.data.toBeValidated.length) {
    ...
    if (email != "") {
      ...
      switch(cards.length)
      {
        ...
        case 1://OK    
          //get name
          let userName = jbCatMan.getUserNamefromCard(cards[0],email);
        ...
        default: //DOUBLE
          ...
          for (let j=0;j<cards.length && memberIdx == -1 ;j++) {
            cats = jbCatMan.getCategoriesfromCard(cards[j]);
          ...
          for (let j=0;j<cards.length;j++) {
            ...
            if (memberIdx == j) {
              menuItem.setAttribute("label", jbCatMan.getUserNamefromCard(cards[j],jbCatMan.bulkEditNoName) + " (*)");
            } else {
              menuItem.setAttribute("label", jbCatMan.getUserNamefromCard(cards[j],jbCatMan.bulkEditNoName));
            }
